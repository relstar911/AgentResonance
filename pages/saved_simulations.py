...
try:
    ...
    st.error(f"Error retrieving simulations: {str(e)}")
    simulations = []
    sim_data = []

# Only show the Load button if we have simulations
if simulations and selected_sim_id is not None:
    if st.button("Load Simulation"):
        try:
            # Fetch the selected simulation data
            loaded_sim_data = database.get_simulation(selected_sim_id)
            
            if loaded_sim_data:
                display_name = loaded_sim_data['name'] if loaded_sim_data['name'] else f"Simulation {loaded_sim_data['id']}"
                ...
                with tab1:
                    ...
                    with col1:
                        ...
                        st.metric("ID", loaded_sim_data['id'])
                        st.metric("Name", loaded_sim_data['name'] if loaded_sim_data['name'] else "Unnamed")
                        st.metric("Date", loaded_sim_data['created_at'].strftime("%Y-%m-%d %H:%M"))
                        st.metric("Grid Size", loaded_sim_data['grid_size'])
                        st.metric("Steps", loaded_sim_data['steps'])
                    
                    with col2:
                        st.subheader("Results Summary")
                        st.metric("Relationship Score", f"{loaded_sim_data['relationship_score']:.3f}")
                        st.metric("Average Resonance", f"{loaded_sim_data['avg_resonance']:.3f}")
                        st.metric("Agent A Avg Bond", f"{loaded_sim_data['avg_bond_strength_a']:.3f}")
                        st.metric("Agent B Avg Bond", f"{loaded_sim_data['avg_bond_strength_b']:.3f}")
                    
                    # Display the description if available
                    if loaded_sim_data['description']:
                        st.subheader("Description")
                        st.markdown(loaded_sim_data['description'])
                    
                    # Delete simulation option
                    if st.button("Delete Simulation", type="primary", key="delete_btn"):
                ...
                with tab2:
                    ...
                    # Load grid data
                    grid = np.array(loaded_sim_data['grid'])
                    
                    # Set up slider for step selection
                    step = st.slider("Step", 0, loaded_sim_data['steps']-1, 0)
                    
                    # Get agent positions for the selected step
                    agent_positions = loaded_sim_data['agent_positions']
                ...
                with tab4:
                    ...
                    path_plot = visualization.plot_agent_paths(log_df, loaded_sim_data['grid_size'])
                ...
                st.error("Could not load simulation data. The simulation may have been deleted.")
        except Exception as e:
            st.error(f"Error loading simulation: {str(e)}")
else:
    if simulations:
        st.warning("Please select a simulation from the list above to view its details.")